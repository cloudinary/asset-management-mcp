...
> {
  const input$: DestroyFolderRequest = {
    folder: folder,
  };

  const parsed$ = safeParse(
    input$,
    (value$) => DestroyFolderRequest$zodSchema.parse(value$),
  ...
  const baseURL$ = options?.serverURL
    || pathToFunc(DestroyFolderOpServerList[0], { charEncoding: "percent" })(
      {
        region: "api",

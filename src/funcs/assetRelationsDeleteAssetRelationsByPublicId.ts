...
import {
  ...
  DeleteAssetRelationsByPublicIdResponse$zodSchema,
...
> {
  const input$: DeleteAssetRelationsByPublicIdRequest = {
  ...
  const parsed$ = safeParse(
    input$,
    (value$) => DeleteAssetRelationsByPublicIdRequest$zodSchema.parse(value$),
  ...
  const [result$] = await M.match<
    ...
    | ConnectionError
  >(
    M.json([200, 207], DeleteAssetRelationsByPublicIdResponse$zodSchema, {
      key: "asset_relations_delete_response",
    }),
    M.json([400, 401, 404], DeleteAssetRelationsByPublicIdResponse$zodSchema, {
      key: "api_error",
    }),

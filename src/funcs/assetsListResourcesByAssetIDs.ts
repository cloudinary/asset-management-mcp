...
import {
  ...
  ListResourcesByAssetIDsResponse$zodSchema,
...
> {
  const input$: ListResourcesByAssetIDsRequest = {
    asset_ids: asset_ids,
  ...
  const parsed$ = safeParse(
    input$,
    (value$) => ListResourcesByAssetIDsRequest$zodSchema.parse(value$),
  ...
  const [result$] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, ListResourcesByAssetIDsResponse$zodSchema, {
      key: "list_response",
    }),
    M.json([400, 401, 404], ListResourcesByAssetIDsResponse$zodSchema, {
      key: "api_error",
    }),
